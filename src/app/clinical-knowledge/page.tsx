...
const DEFAULT_SUBJECTS: SubjectItem[] = [
  ...
  { id: "pharmacology", name_en: "Pharmacology", name_ar: "علم الأدوية", desc_en: "Pharmacokinetics & general drug principles", desc_ar: "حركية الأدوية والعلوم السريرية للعلاجات" },
...
export default function ClinicalKnowledgePage() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem("medicinety_clinical_list");
    if (saved) {
      try {
  ...
  const saveSubjects = (newList: SubjectItem[]) => {
    setSubjects(newList);
    localStorage.setItem("medicinety_clinical_list", JSON.stringify(newList));
  };
          ...
          <div className="flex-1">
            ...
            <div className="mb-2">
              <h1 className="text-3xl md:text-4xl font-extrabold text-black dark:text-white tracking-tight">
                {language === "ar" ? "العلوم السريرية" : "Clinical Knowledge"}
              </h1>
            </div>
            <p className="text-sm text-slate-700 dark:text-slate-300 font-bold max-w-xl">
              {language === "ar" ? "تدريب سريري متقدم ومساقات التحضير للامتياز والبورد الطبي" : "Advanced Clerkships, Rotations & Clinical Board Preparation"}
            </p>
          </div>
        </section>
          ...
          <div className="flex justify-between items-center border-b border-slate-100 dark:border-teal-500/10 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-bold text-lg md:text-xl text-black dark:text-white tracking-tight">
                {language === "ar" ? "التخصصات والأقسام السريرية" : "Clinical Specialties & Rotations"}
              </h2>
            </div>
